...
class MrpProduction(models.Model):
    ...
    backorder_sequence = fields.Integer("Backorder Sequence", default=0, copy=False,
    ...
    product_variant_attributes = fields.Many2many('product.template.attribute.value',
    ...
    production_duration_expected = fields.Float("Expected Duration", help="Total expected duration (in minutes)",
                                                compute='_compute_production_duration_expected')
    production_real_duration = fields.Float("Real Duration", help="Total real duration (in minutes)",
    ...
    delay_alert_date = fields.Datetime('Delay Alert Date', compute='_compute_delay_alert_date',
    ...
    mrp_production_backorder_count = fields.Integer("Count of linked backorder",
                                                    compute='_compute_mrp_production_backorder')
    ...
    production_capacity = fields.Float(compute='_compute_production_capacity',
    ...
    show_serial_mass_produce = fields.Boolean('Display the serial mass produce wizard action',
    ...
    def _compute_bom_id(self):
        ...
        for company_id, productions in mo_by_company_id.items():
            ...
            boms_by_product = self.env['mrp.bom'].with_context(active_test=True)._bom_find(productions.product_id,
    ...
    def _compute_production_capacity(self):
        for production in self:
            ...
            if moves:
                production_capacity = min(moves.mapped(
                    lambda move: move.product_id.uom_id._compute_quantity(move.product_id.qty_available,
                                                                          move.product_uom) / move.unit_factor))
                production.production_capacity = min(production.product_qty, float_round(production_capacity,
        ...
        for production in self:
            if production.workorder_ids:
                production.is_planned = any(
                    wo.date_planned_start and wo.date_planned_finished for wo in production.workorder_ids)
            else:
    ...
    def _compute_delay_alert_date(self):
        delay_alert_date_data = self.env['stock.move']._read_group(
            [('id', 'in', self.move_raw_ids.ids), ('delay_alert_date', '!=', False)], ['delay_alert_date:max'],
            'raw_material_production_id')
        delay_alert_date_data = {data['raw_material_production_id'][0]: data['delay_alert_date'] for data in
        ...
        for production in (self - production_no_alert):
            production.json_popover = json.dumps({
                ...
                'late_elements': [{
                    ...
                    lambda m: m.delay_alert_date).move_orig_ids._delay_alert_get_documents()
                ]
            })
    ...
    def _compute_confirm_cancel(self):
        ...
        res = self.env['stock.move'].read_group(domain, ['state', 'production_id', 'raw_material_production_id'],
                                                ['production_id', 'raw_material_production_id'], lazy=False)
    ...
    def _compute_scrap_move_count(self):
        data = self.env['stock.scrap']._read_group([('production_id', 'in', self.ids)], ['production_id'],
                                                   ['production_id'])
    ...
    def _compute_show_serial_mass_produce(self):
        ...
        for order in self:
            if order.state == 'confirmed' and order.product_id.tracking == 'serial' and \
                    ...
                    float_compare(order.qty_producing, order.product_qty,
                                  precision_rounding=order.product_uom_id.rounding) < 0:
    ...
    def _compute_forecasted_issue(self):
        for order in self:
            ...
            if order.product_id:
                virtual_available = order.product_id.with_context(warehouse=warehouse.id,
                                                                  to_date=order.date_planned_start).virtual_available
        ...
        for production in self:
            ...
            if vals.get('date_planned_start'):
                production.move_raw_ids.write(
                    {'date': production.date_planned_start, 'date_deadline': production.date_planned_start})
            ...
            if production.state == 'done' and ('lot_producing_id' in vals or 'qty_producing' in vals):
                finished_move_lines = production.move_finished_ids.filtered(
                    lambda move: move.product_id == production.product_id and move.state == 'done').mapped(
                    'move_line_ids')
                ...
                production.move_finished_ids.filtered(
                    lambda m: m.product_id == production.product_id).move_line_ids.write(
                    {'lot_id': production.lot_producing_id.id})
            if self._has_workorders() and not production.workorder_ids.operation_id and vals.get(
                    'date_planned_start') and not vals.get('date_planned_finished'):
        ...
        for vals in vals_list:
            ...
            if vals.get('move_finished_ids', False) and vals.get('move_byproduct_ids', False):
                vals['move_finished_ids'] = list(
                    filter(lambda move: move[2]['product_id'] == vals['product_id'], vals['move_finished_ids']))
                vals['move_finished_ids'] = vals.get('move_finished_ids', []) + vals['move_byproduct_ids']
                del vals['move_byproduct_ids']
    ...
    def copy_data(self, default=None):
        ...
        if not default or 'move_raw_ids' not in default:
            default['move_raw_ids'] = [(0, 0, move.copy_data()[0]) for move in
                                       self.move_raw_ids.filtered(lambda m: m.product_qty != 0.0)]
    ...
    def action_view_mo_delivery(self):
        ...
        if len(self.picking_ids) > 1:
            ...
            action['views'] = picking_form_view + [(state, view) for state, view in action.get('views', []) if
                                                   view != 'form']
    ...
    def _create_update_move_finished(self):
        ...
        for move_finished_values in moves_finished_values:
            if move_finished_values.get('byproduct_id') in moves_byproduct_dict:
                # update existing entries
                list_move_finished += [
                    Command.update(moves_byproduct_dict[move_finished_values['byproduct_id']].id, move_finished_values)]
    ...
    def _get_sources(self):
        ...
        return (dest_moves | parent_moves).group_id.mrp_production_ids.filtered(
            lambda p: p.origin != self.origin) - self
    ...
    def _link_workorders_and_moves(self):
        ...
        for move in (self.move_raw_ids | self.move_finished_ids):
            if move.operation_id:
                move.write({
                    'workorder_id': workorder_per_operation[
                        move.operation_id].id if move.operation_id in workorder_per_operation else False
                })
            else:
                bom = move.bom_line_id.bom_id if (
                            move.bom_line_id and move.bom_line_id.bom_id in workorder_boms) else self.bom_id
                move.write({
    ...
    def _get_consumption_issues(self):
        ...
        for order in self:
            ...
            for move in order.move_raw_ids:
                ...
                if not (move.product_id in expected_qty_by_product or float_is_zero(qty_done,
        ...
        for production in self:
            ...
            if finish_moves:
                production._log_downside_manufactured_quantity(
                    {finish_move: (production.product_uom_qty, 0.0) for finish_move in finish_moves}, cancel=True)
        ...
        for order in self:
            ...
            for move in finish_moves:
                if move.quantity_done:
                    continue
                move._set_quantity_done(float_round(order.qty_producing - order.qty_produced,
    ...
    def _split_productions(self, amounts=False, cancel_remaining_qty=False, set_consumed_qty=False):
        ...
        for production in self:
            ...
            diff = float_compare(production.product_qty, total_amount,
                                 precision_rounding=production.product_uom_id.rounding)
        ...
        for initial_move, backorder_moves in move_to_backorder_moves.items():
            ...
            if (initial_move.raw_material_production_id or (
                    initial_move.production_id and initial_move.product_id != production.product_id)) \
                    and not initial_move.move_line_ids and set_consumed_qty:
                ml_vals = initial_move._prepare_move_line_vals()
                backorder_move_to_ignore = backorder_moves[-1] if has_backorder_to_ignore[
                    initial_move.raw_material_production_id] else self.env['stock.move']
        ...
        for production in self:
            ...
            for workorder in production.workorder_ids:
                initial_workorder_remaining_qty.append(
                    max(initial_qty - workorder.qty_reported_from_previous_wo - workorder.qty_produced, 0))
    ...
    def button_scrap(self):
        ...
        return {
            ...
            'context': {'default_production_id': self.id,
                        'product_ids': (self.move_raw_ids.filtered(
    ...
    def _log_downside_manufactured_quantity(self, moves_modification, cancel=False):
        ...
        documents = self.env['stock.picking']._log_activity_get_documents(moves_modification, 'move_dest_ids', 'DOWN',
                                                                          _keys_in_groupby)
        documents = self.env['stock.picking']._less_quantities_than_expected_add_documents(moves_modification,
                                                                                           documents)
    ...
    def _log_manufacture_exception(self, documents, cancel=False):
        ...
        def _render_note_exception_quantity_mo(rendering_context):
            ...
            if visited_objects:
                ...
                impacted_object = visited_objects.filtered(lambda m: m.state not in ('done', 'cancel')).mapped(
                    'picking_id')
            values = {
                'production_order': self,
    ...
    def action_serial_mass_produce_wizard(self):
        ...
        if self.reservation_state != 'assigned':
            missing_components = {move.product_id for move in self.move_raw_ids if
                                  float_compare(move.reserved_availability, move.product_uom_qty,
        ...
        action['context'] = {
            ...
            'default_multiple_lot_components_names': ",".join(
                c.display_name for c in multiple_lot_components) if multiple_lot_components else None,
        }
        return action
    ...
    def _check_immediate(self):
        ...
        for production in self:
            if all(float_is_zero(ml.qty_done, precision_digits=pd) for
                   ml in production.move_raw_ids.filtered(
                lambda m: not m.manual_consumption and m.state not in ('done', 'cancel')).move_line_ids
